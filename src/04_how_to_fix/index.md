...
修正したいオブジェクトを選択し、<u>オブジェクトデータプロパティ</u>から、 <u>形状データ</u>内の <u>カスタム分割法線データを削除</u> ボタンを押すことで、修復できます。
...
Basisにあたる頂点を大規模にいじってしまったので、当然の如くシェイプキーが全壊します。これは、自動でも手動でも変わらずです。

ブレンドシェイプからデシメートを設定する場合は、適用していない間は壊れていないように見えることがあります。
しかし、結局の所、シェイプキー適用後は確実に壊れます。なぜなのか。

また、破損したシェイプキーの修復方法で調べてみると、情報が結構出てくるようですが、
これらの情報は全て **頂点数を減していない** 前提であるため、今回のケースにおいては概ね役に立ちません。
...
方法は３つ有りますが、それぞれ条件によって使える方法が異なります。具体的には以下の通り。[^4-1]

[^4-1]: シェイプキーの修復方法の紹介にて、Blenderの基本機能である <u>シェイプキーの転送</u> については紹介できませんでした。頂点数が異なり、元の形状も異なるオブジェクトに対して転送しようとすると、エラーになってしまいました。残念です。
...
- Body
    - vrc.blink_left .. 方法1
    - vrc.blink_right .. 方法1
    - vrc.lowerlid_left .. 方法1
    - vrc.lowerlid_right .. 方法1
    - それ以外 .. 方法3
 - Costume
    - Costume_2_bust .. 方法2
...
シェイプキーの影響を受ける頂点を、ポリゴン削減作業で減らしていないのであれば、<u>モディファイア</u>から <u>シュリンクラップ</u>が使えます。
機械的な対処なので確実ではないですが、運が良ければ手作業なし、運河よくなくても少ない手直しだけで出来上がるので、**比較的** 楽です。[^4-2]

口元や表情などの頂点が密集している箇所でこの方法をつかうと、シュリンクラップがうまくいかなかったり、仮にうまくできたとしても、メッシュが汚かったりします。
