...
前提として知っておくべきは、 **シェイプキーが全部壊れる** ことです。これは機械的でも手動でも発生します。

また、[<u>Cats Blender Plugin</u>](https://github.com/GiveMeAllYourCats/cats-blender-plugin)  という、機械的なデシメートにシェイプキーを維持する機能があるプラグインも存在しますが、
今回のような削減量が多いケースにおいては、うまく適用することが出来ませんでした。

つまり、ポリゴン削減の作業は、事前に **作業後に復元するシェイプキーの全貌と、それぞれどのように復元するかを把握** し、それを考慮した削減をする必要があります。
シェイプキーの修復方法については、次章にて解説があります。

たとえば、表情表現用のシェイプキーを復元する場合、顔のメッシュのポリゴン削減については、以下に気をつけます

- 口元や目の周りの頂点を削りすぎない
- 八重歯など、デティールが細かすぎる表現は逆にまるごとなくしてしまう
- **機械的に作業すると復元作業ができない**
...
<dl>
    ...
    <dd>メッシュの形を変形して、透過なしでも使えるように治す .. Blender</dd>
    <dt> 半透過テクスチャや半透過のシェーダーが使われている場合 </dt>
    <dd>
        <dl>
            <dt>服の一部などに使われているもの </dt>
            <dd> 非透過でも違和感がないようにテクスチャを修正する .. Photoshop (Gimp 他)</dd>
            <dt> 顔の照れ顔表現など、アニメーションに使用する場合 </dt>
            <dd>
                シェイプキーで動かすメッシュを削除する ... Blender<br />
                アニメーションで使用するテクスチャを変更する ... Unity, Photoshop (Gimp 他)
            </dd>
        </dl>
    </dd>
</dl>
...
見えないポリゴン・見えづらいポリゴンを事前に手動で消してしまってから自動削減すると、効率よく削減できます。
